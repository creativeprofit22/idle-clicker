...
Prestige still needs a Legacy calculation, reset eligibility, repeat-milestone reward rules, the first permanent upgrade, and a final retention list. **Update (23 Sep 2026, user-approved):** the Legacy calculation, reset eligibility and repeat rewards are chosen as above, and the first permanent upgrade is **Drill**: ranks 1/2/3 cost 10/20/40 Legacy and multiply squad damage by 2×/3×/4×, leaving health, gold, round speed and enemies unchanged. Legacy and Drill rank survive every reset; the final retention list is still open. “Unlocked traditions survive” and “troop levels reset” can coexist, but the implementation will need to distinguish access to a unit type from the temporary strength or ownership of troops in a run. Similarly, a veteran starting frontline is an example of a purchased permanent benefit, not a promise that all purchased troops survive every reset.
