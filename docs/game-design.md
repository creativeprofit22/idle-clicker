...
Legacy would come mainly from the highest secured campaign milestone, with a visible preview of what reaching the next milestone earns. This ties prestige to completing the attack-and-defense rhythm rather than merely reaching an arbitrary damage total. Its intended incentive is a considered decision about when a campaign has run its course, rather than an optimal strategy of resetting every ninety seconds. The precise formula, repeat-run rewards, and reset availability still need to be chosen. **Update (23 Sep 2026, user-approved):** securing a campaign pays 10 Legacy the first time (dynasty 1) and 3 Legacy in every later dynasty; Found a Dynasty is available after every secured campaign, with no limit. **Superseded (23 Sep 2026) by Threat:** when founding a later dynasty the player picks its **Threat** level (0 by default, at most one above the best Threat already secured, capped at 10). Each level gives enemies +25% health and damage for that whole dynasty, and securing a later dynasty now pays **3 × (1 + Threat)** Legacy (3/6/9…). Dynasty 1 still pays 10 at Threat 0. See `first-playable.md` for the exact rules.
...
For example, a logistics doctrine might automatically buy early troop levels, while a veteran cadre might provide an established frontline at the start of a dynasty. These are possible expressions of institutional memory, not an approved automation tree. They would reduce repeated setup and make the army feel as though it retained what previous campaigns learned. **Update (24 Sep 2026, user-approved): Veteran Cadre.** A second permanent Legacy upgrade: one rank, bought once for **50 Legacy**. Every dynasty founded after the purchase starts Shield infantry, Foot archers and Horse archers at **level 2** instead of 1; the current dynasty and its battle are unchanged. The gate still starts at level 1, and Drill, Threat, Legacy payouts, gold rewards and the away reward are unaffected. Logistics automation and repeatable Legacy sinks remain deferred. See `first-playable.md` for the exact rules.
...
Prestige still needs a Legacy calculation, reset eligibility, repeat-milestone reward rules, the first permanent upgrade, and a final retention list. **Update (23 Sep 2026, user-approved):** the Legacy calculation, reset eligibility and repeat rewards are chosen as above, and the first permanent upgrade is **Drill**: ranks 1/2/3 cost 10/20/40 Legacy and multiply squad damage by 2×/3×/4×, leaving health, gold, round speed and enemies unchanged. Legacy and Drill rank survive every reset, as do the best secured Threat and total Legacy earned; the final retention list is still open. “Unlocked traditions survive” and “troop levels reset” can coexist, but the implementation will need to distinguish access to a unit type from the temporary strength or ownership of troops in a run. Similarly, a veteran starting frontline is an example of a purchased permanent benefit, not a promise that all purchased troops survive every reset.

Closed-app progression needs its cap, reward rate, timing policy, and first-slice inclusion decided. *(Update: the campaign prototype now uses the defaults recorded under "Offline behavior and closed-app progression"; they are open to tuning.)* Open-session idling at strongholds or counterattacks is separately unresolved. The proposed distinction is deliberate: playable without internet, automatic combat while open, and rewards while closed are three different behaviors.
