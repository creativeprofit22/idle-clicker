...
# Headless, session-local conquest. The playable scene still uses ordinary Economy.
enum Phase { RUNNING, CONQUEST_CLEARED }
...
func _begin_encounter(encounter: int) -> Combat:
	return super.restart_battle(encounter)
