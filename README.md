...
| G6 | **PASSED (window-driven)** | `python tests/windows_g6_driver.py` on `18d138e`: 23 checks, 0 failures, driver exit 0 (two consecutive runs; final one after the capture-numbering fix), both launches exited 0 on WM_CLOSE with child reaped and reader joined. The driver plays the real campaign scene by clicks posted only to the launched PID's window (buy cheapest upgrade, gate before defense, retry frontier), using a read-only UI reporter autoload that exists only in the disposable copy. Dynasty 1 secured after 126 s of real play: "+10 Legacy earned", Legacy 10. Train Drill saved `legacy` 0, `drill_rank` 1 at once. Preview listed kept Legacy 0/Drill rank 1; Cancel left `campaign.json` byte-identical; Confirm → dynasty 2, damage 8/16/12. Passive Border 72→36→0 in 1.94 s, +10. Relaunch kept "Dynasty 2 · Legacy 0 · Drill rank 1". Dynasty 2 secured after 20 s: "+3 Legacy earned", Found a Dynasty enabled; Confirm → dynasty 3 keeping Legacy 3, Drill rank 1. Copy's `progress.json` identical throughout. Captures inspected (secured + preview screens for dynasties 1 and 2: correct text, no overlap). Copy: `%LOCALAPPDATA%\Temp\bs-g6-18d138e-150744`. |
